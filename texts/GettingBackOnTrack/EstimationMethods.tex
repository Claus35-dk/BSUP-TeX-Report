...
Planning Poker is within SCRUM to estimate time of tasks at the backlog \textbf{\cite[p. ?]{?}}. Planning Poker is usable within the WBS/PBS paradigm as well.
...
<<<<<<< HEAD
The last estimation method we have thought about is the PERT estimation method\cite[p. 152 ]{PM}. While most other methods only ends up with one estimation for each task, the PERT method uses three estimates, and then calculate it into one. The force of the PERT method lies in its three estimates:
=======
The last estimation method we have thought about is the PERT estimation method. While most other methods only ends up with one estimation for each task, the PERT method uses three estimates, and then calculate it into one. The force of the PERT method lies in its three estimates\textbf{\cite[p. ?]{?}}:
>>>>>>> baa7616f10b10a39ecfa62350c7464feae602a2d
